...
NS_SRC2="ns_src2"
SRC21_OUT_VETH="src21-out"
SRC21_IN_VETH="src21-in"
SRC22_OUT_VETH="src22-out"
SRC22_IN_VETH="src22-in"
...
sudo ip link del $SRC22_OUT_VETH
sudo ip link del $SRC22_IN_VETH
...
sudo ip link add $SRC22_OUT_VETH type veth peer name $SRC22_IN_VETH
...
sudo ip link set $SRC22_OUT_VETH netns $NS_SRC2
...
sudo ip netns exec $NS_SRC2 ip address add 10.55.0.10/24 dev $SRC22_OUT_VETH
sudo ip netns exec $NS_SRC2 ip link set dev $SRC22_OUT_VETH up
sudo ip address add 10.55.0.20/24 dev $SRC22_IN_VETH
sudo ip link set $SRC22_IN_VETH up
...
sudo ip netns exec $NS_SRC2 ip route add 192.168.53.0/24 via 10.54.0.20 table 1

sudo ip netns exec $NS_SRC2 ip rule add from 10.55.0.10 table 2
sudo ip netns exec $NS_SRC2 ip route add 192.168.53.0/24 via 10.55.0.20 table 2
...
sudo ip route add 192.168.53.0/24 via 192.168.53.10 table 4

sudo ip rule add from 10.55.0.20 table 5
sudo ip route add 192.168.53.0/24 via 192.168.53.10 table 5
...
# In this scenario this path SHOULD NOT BE USED!!!!!!!!!
# If it is used, then it should be immediately visible in the result
BW=99999
LATENCY=100
BURST=15400
