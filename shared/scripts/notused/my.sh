...
sudo ip link set dev $S1 up
sudo ip address add 10.0.0.1/30 dev $S1
...
sudo ip link set dev $R1 up
sudo ip address add 10.0.0.6/30 dev $R1

#Add ip routes
#sudo ip netns exec $NS_MID sudo ip route add 10.0.0.4/30 via 10.0.0.5
#sudo ip netns exec $NS_MID sudo ip route add 10.0.0.0/30 via 10.0.0.2

sudo ip route add 10.0.0.4/30 via 10.0.0.2
sudo ip route add 10.0.0.0/30 via 10.0.0.5

#sudo ip netns exec $NS_MID sudo ip rule add from 10.0.0.2 table 1
#sudo ip netns exec $NS_MID sudo ip route add 10.0.0.4/30 dev $M2R1 scope link table 1
#sudo ip netns exec $NS_MID sudo ip rule add from 10.0.0.5 table 2
#sudo ip netns exec $NS_MID sudo ip route add 10.0.0.0/30 dev $S2M1 scope link table 2
